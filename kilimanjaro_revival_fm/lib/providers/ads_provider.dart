...
class AdsProvider extends ChangeNotifier {
  ...
  Future<void> loadAds() async {
    ...
    try {
      ...
      if (result['success'] == true) {
        _ads = (result['ads'] as List).map((json) {
          print(
            '📥 Raw Ad JSON: ${json['title']} - Image: ${json['image'] ?? json['image_url'] ?? json['image_display']}',
          );
  ...
  Future<Map<String, dynamic>> createAd(
    ...
    try {
      final result = await _apiService.createAd(
        ad.toJson(),
        imageFile: imageFile,
      );
      ...
      _isLoading = false;
      notifyListeners();
      return {'success': false, 'error': 'Failed to create ad: $e'};
    }
  }
  ...
  Future<Map<String, dynamic>> updateAd(
    ...
    try {
      final result = await _apiService.updateAd(
        id,
        ad.toJson(),
        imageFile: imageFile,
      );
      ...
      _isLoading = false;
      notifyListeners();
      return {'success': false, 'error': 'Failed to update ad: $e'};
    }
  }
  ...
  Future<Map<String, dynamic>> deleteAd(int id) async {
    ...
    try {
      ...
      _isLoading = false;
      return {'success': false, 'error': 'Failed to delete ad: $e'};
    }
  }
